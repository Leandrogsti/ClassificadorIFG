...
4. Inclua quem foi atingido por bala perdida, marcando bala_perdida como true.
  ...
  "vitimas": [
    {
      ...
      "situacao": "morta|ferida|ilesa",
      "data_morte": "AAAA-MM-DD ou null",
      "cargo_politico": "cargo/candidatura ou null",
      "bala_perdida": false,
      "circunstancia": "como foi atingida, em uma frase",
      "fonte": "Notícia 1",
...
def montar_prompt_sistema() -> str:
    ...
    return (
        dicionario
        + "\n"
        + INSTRUCOES_VITIMAS.format(skills=_skills_indicadores())
        + "\n"
        + FORMATO
    )
...
def _normalizar_vitima(bruta: dict) -> dict:
    ...
    return {
        ...
        "bala_perdida": 1 if bruta.get("bala_perdida") else 0,
        "circunstancia": texto(bruta.get("circunstancia")),
...
def mesma_pessoa(a: dict, b: dict) -> bool:
    ...
    nome_a = (a.get("nome") or "").strip().lower()
    nome_b = (b.get("nome") or "").strip().lower()
